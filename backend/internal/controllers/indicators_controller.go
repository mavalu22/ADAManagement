...
type DashboardData struct {
	...
	NearGraduationStudents []NearGraduationStudent `json:"near_graduation_students"` // Novo Campo
}
...
// Struct para Alunos Críticos (Risco)
...
// Nova Struct para Alunos na Reta Final
...
func GetDashboardIndicatorsHandler(c *gin.Context) {
	...
	// 2. Alunos Críticos (Locks > 1 OR SemestersNoHours > 1)
	...
	// 3. Alunos na Reta Final (Matérias Pendentes <= 6)
	database.DB.Table("academic_records").
		...
		Where("academic_records.semester_id = ?", semesterID).
		Where("academic_records.status = ?", "Em regularidade"). // Apenas ativos
		Where("academic_records.pending_obligatory <= ?", 6).    // Filtro principal
		Order("academic_records.pending_obligatory ASC").        // Quem tem menos falta primeiro
